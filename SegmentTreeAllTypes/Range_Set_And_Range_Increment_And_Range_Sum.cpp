...
pair<long long int,long long int> Upd[2000005];  // pair of Value to Set, Value to Add
...
long long int getSum(long long int idx,long long int lo,long long int hi,long long int left,long long int right)
{
    if(Upd[idx].first!=0)
    {
        long long int val=Upd[idx].first;
        long long int cnt=hi-lo+1;
        Seg[idx]=cnt*val;
        Upd[idx].first=0;
        Upd[2*idx+1].second=0;
        Upd[2*idx+2].second=0;
        Upd[2*idx+1].first=val;
        Upd[2*idx+2].first=val;
    }
    if(Upd[idx].second!=0)
    {
        long long int val=Upd[idx].second;
        long long int cnt=hi-lo+1;
        Seg[idx]+=cnt*val;
        Upd[idx].second=0;
        Upd[2*idx+1].second+=val;
        Upd[2*idx+2].second+=val;
    }
...
void update(long long int idx,long long int lo,long long int hi,long long int left,long long int right,long long int val)
{
    ...
    if(lo>=left && hi<=right)
    {
        Seg[idx]+=val*(hi-lo+1);
        if(lo!=hi)
            Upd[2*idx+1].second+=val,Upd[2*idx+2].second+=val;
        return;
    }
    long long int mid=lo+(hi-lo)/2;
...
void update2(long long int idx,long long int lo,long long int hi,long long int left,long long int right,long long int val)
{
    // if(Upd[idx]!=0)
    // {
    //     long long int val1=Upd[idx];
    ...
    long long int mid=lo+(hi-lo)/2;
    update2(2*idx+1,lo,mid,left,right,val);
    update2(2*idx+2,mid+1,hi,left,right,val);
    Seg[idx]=Seg[2*idx+1]+Seg[2*idx+2];
