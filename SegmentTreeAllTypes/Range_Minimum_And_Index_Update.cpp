...
public:
    ...
    int minSeg(int v, int tl, int tr, int l, int r)
    {
        if (l > r) 
            return INT_MAX;
        if (l == tl && r == tr) {
        ...
        int tm = (tl + tr) / 2;
        return min(minSeg(v*2, tl, tm, l, min(r, tm)),minSeg(v*2+1, tm+1, tr, max(l, tm+1), r));
    }
    ...
    int minRange(int left, int right) {
        return minSeg(1,0,n-1,left,right);
